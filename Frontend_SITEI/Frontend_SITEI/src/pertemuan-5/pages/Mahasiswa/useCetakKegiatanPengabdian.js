...
/**
 * @param {Object} opts
 * @param {Array}  opts.services       — data kegiatan pengabdian dalam rentang tanggal
 ...
 */
export const cetakKegiatanPengabdianPDF = async ({
    services      = [],
    resident      = {},
    konsulen      = {},
    tanggalDari   = '',
    ...
    await drawCoverPage(doc, { services, resident, konsulen, tanggalDari, tanggalSampai });

    doc.addPage();
    await drawTablePages(doc, { services, resident, konsulen, tanggalDari, tanggalSampai });

    const safeName = (resident.identifier || 'residen').replace(/[\s/\\]/g, '_');
    doc.save(`Logbook_KegiatanPengabdian_${safeName}_${tanggalDari}_sd_${tanggalSampai}.pdf`);
};
...
async function drawCoverPage(doc, { services, resident, konsulen, tanggalDari, tanggalSampai }) {
    ...
    doc.setFontSize(10);
    doc.setTextColor(...BLACK);
    doc.text('LOGBOOK KEGIATAN PENGABDIAN MASYARAKAT RESIDEN', ML + 60, judulY + 1.5, { align: 'center' });

    doc.setFontSize(9);
    doc.text('MEDLOG-PM', ML + 145, judulY + 1.5, { align: 'center' });
    ...
    const totalKegiatan  = services.length;
    const totalVerified  = services.filter(s => s.status === 'verified').length;
    const totalPending   = services.filter(s => s.status === 'pending').length;
    const totalRejected  = services.filter(s => s.status === 'rejected').length;

    const statsData = [
        ['Total Kegiatan Pengabdian', String(totalKegiatan)],
    ...
    const qrResData = [
        ...
        `TOTAL:${services.length}`, 'PENGABDIAN'
    ].join('|');
    ...
    doc.setFontSize(7);
    doc.setTextColor(...BLACK);
    doc.text('KONSULEN / DPJP', ktx, ttdY + 8);
...
// ─────────────────────────────────────────────────────────────────
//  HALAMAN 2+ — TABEL DETAIL KEGIATAN PENGABDIAN
// ─────────────────────────────────────────────────────────────────

async function drawTablePages(doc, { services, resident, konsulen, tanggalDari, tanggalSampai }) {
    ...
    const drawPageHeader = () => {
        ...
        doc.setFontSize(9);
        doc.setTextColor(...BLACK);
        doc.text('DETAIL KEGIATAN PENGABDIAN MASYARAKAT — LOGBOOK ANESTESIOLOGI', ML, MT + 7);
    ...
    const drawPageFooter = (pageNum) => {
        ...
        doc.setFontSize(6.5);
        doc.setTextColor(...GRAY50);
        doc.text('Prodi Anestesiologi · Logbook Kegiatan Pengabdian Masyarakat Anestesiologi dan Terapi Intensif FK UNRI', ML, fy + 6);
    ...
    autoTable(doc, {
        ...
        didDrawCell: (data) => {
            if (data.column.index === 5 && data.section === 'body') {
                ...
                doc.setFontSize(5.5);
                doc.setTextColor(...BLACK);
                doc.text(status, bx + bw / 2, by + bh / 2 + 1, {
                    align: 'center'
                });
            }
        },
...
export default cetakKegiatanPengabdianPDF;