...
/**
 * @param {Object} opts
 * @param {Array}  opts.guidances      — data bimbingan dalam rentang tanggal
 ...
 */
export const cetakBimbinganKonselingPDF = async ({
    guidances     = [],
    resident      = {},
    konsulen      = {},
    tanggalDari   = '',
    ...
    await drawCoverPage(doc, { guidances, resident, konsulen, tanggalDari, tanggalSampai });

    doc.addPage();
    await drawTablePages(doc, { guidances, resident, konsulen, tanggalDari, tanggalSampai });

    const safeName = (resident.identifier || 'residen').replace(/[\s/\\]/g, '_');
    doc.save(`Logbook_BimbinganKonseling_${safeName}_${tanggalDari}_sd_${tanggalSampai}.pdf`);
};
...
async function drawCoverPage(doc, { guidances, resident, konsulen, tanggalDari, tanggalSampai }) {
    ...
    doc.setFontSize(10);
    doc.setTextColor(...BLACK);
    doc.text('LOGBOOK BIMBINGAN & KONSELING RESIDEN', ML + 60, judulY + 1.5, { align: 'center' });

    doc.setFontSize(9);
    doc.text('MEDLOG-BK', ML + 145, judulY + 1.5, { align: 'center' });
...
async function drawTablePages(doc, { guidances, resident, konsulen, tanggalDari, tanggalSampai }) {
    ...
    const drawPageHeader = () => {
        ...
        doc.setFontSize(9);
        doc.setTextColor(...BLACK);
        doc.text('DETAIL CATATAN BIMBINGAN & KONSELING — LOGBOOK ANESTESIOLOGI', ML, MT + 7);
    ...
    const drawPageFooter = (pageNum) => {
        ...
        doc.setFontSize(6.5);
        doc.setTextColor(...GRAY50);
        doc.text('Prodi Anestesiologi · Logbook Bimbingan & Konseling Anestesiologi dan Terapi Intensif FK UNRI', ML, fy + 6);
    ...
    autoTable(doc, {
        ...
        head: [[
            'No',
            'Tanggal',
            'Keterangan Bimbingan & Konseling',
            'Konsulen / DPJP',
            'Paraf',
        ]],
        body: rows,
...
export default cetakBimbinganKonselingPDF;