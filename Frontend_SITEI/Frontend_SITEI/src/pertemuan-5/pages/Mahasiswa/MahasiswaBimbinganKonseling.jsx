...
import {
    HeartHandshake, Calendar, Inbox, Loader2, Filter,
    Info, ShieldCheck, Printer, Download, X, RefreshCcw
} from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

// ── Import PDF utility ────────────────────────────────────────────
import { cetakBimbinganKonselingPDF } from './useCetakBimbinganKonseling';
...
const CetakModal = ({ guidances, cases = [], onClose }) => {
    ...
    // Ambil nama konsulen:
    // Prioritas 1 — field konsulen_name di data bimbingan (jika backend mengisi)
    // Prioritas 2 — dpjp_name dari riwayat kasus (sama dengan konsulen pembimbing)
    // Prioritas 3 — fallback '—'
    const firstKonsulen =
        guidances.find(g => g.konsulen_name)?.konsulen_name ||
    ...
    const handleCetak = async () => {
        ...
        if (filteredData.length === 0) {
            Swal.fire('Tidak Ada Data', 'Tidak ada catatan bimbingan & konseling pada rentang tanggal yang dipilih.', 'info');
            return;
        }

        setLoading(true);
        try {
            await cetakBimbinganKonselingPDF({
                    ...
                    <div className="bg-[#003178] p-6 text-white flex justify-between items-start">
                        <div>
                            ...
                            <div className="text-lg font-black uppercase flex items-center gap-2">
                                <Printer size={18} /> Cetak Logbook Bimbingan & Konseling
                            </div>
                            <div className="text-xs opacity-70 mt-1">
                                Hanya catatan VERIFIED yang akan dicetak
                            </div>
                        </div>
                    ...
                    <div className="p-6 space-y-5">
                                ...
                                <div className={`rounded-2xl p-4 border flex items-center gap-3 ${
                                    ...
                                    <div>
                                        <div className={`text-xs font-black ${
                                            ...
                                            {filteredData.length > 0
                                                ? `${filteredData.length} catatan bimbingan & konseling siap dicetak`
                        ...
                        <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 space-y-1.5">
                            ...
                            {[
                                ...
                                'Halaman 1: Kop FK UNRI + identitas residen + QR TTD digital',
                                'Halaman 2+: Tabel detail seluruh catatan bimbingan & konseling',
                                'QR Code berisi identitas digital residen & konsulen pembimbing',
                                'Keterangan bimbingan & konseling residen tercatat lengkap',
                            ].map((info, i) => (
...
export default function MahasiswaBimbinganKonselingView() {
    ...
    const fetchStudentGuidances = useCallback(async () => {
        try {
            setIsLoading(true);
            // Mengambil data dari endpoint riwayat bimbingan milik mahasiswa yang sedang login
            const res = await axios.get('https://api.sigmaeducation.id/api/mahasiswa/guidance-counselings', {
                headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
            });
            setGuidances(Array.isArray(res.data) ? res.data : []);

            // Fetch riwayat kasus untuk mendapatkan dpjp_name sebagai fallback konsulen
            try {
            ...
            console.error('Gagal memuat logbook bimbingan & konseling:', err);
            setGuidances([]);
        } finally {
            setIsLoading(false);
        }
    ...
    if (isLoading) return (
        <div className="w-full h-screen flex flex-col items-center justify-center bg-slate-50 gap-2">
            ...
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Memuat Lembar Konseling...</p>
        </div>
    );
        ...
        <div className="p-6 bg-[#F8FAFC] min-h-screen font-['Manrope']">
            ...
            <div className="mb-6 flex items-start justify-between gap-4">
                <div>
                    ...
                    <p className="text-xs font-bold text-slate-400 mt-1 uppercase tracking-tight">
                        Pengembangan Penalaran, Minat, Bakat, Seni, dan Kesejahteraan Residen
                    </p>
                </div>
            ...
            <div className="bg-blue-50/50 border border-blue-100 rounded-2xl p-4 mb-6 flex items-start gap-3">
                ...
                <div className="text-xs font-medium text-blue-700 leading-relaxed">
                    ...
                    Halaman ini bersifat <span className="font-extrabold">Read-Only</span>. Lembar rekam jejak bimbingan konseling dan kesejahteraan residen ini diinput, diperbarui, serta disahkan secara mandiri langsung oleh Dosen Konsulen/Bimbingan Anda.
                </div>
            </div>
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200/60 shadow-sm mb-6">
                <div className="flex gap-3 items-center">
                    <div className="relative flex-1">
                        ...
                        <input
                            type="text"
                            placeholder="Saring berdasarkan deskripsi atau uraian keterangan bimbingan..."
                    ...
                    <table className="w-full text-left border-none">
                        <thead className="bg-slate-50/60 text-slate-400 text-[10px] font-black uppercase border-b border-slate-100 tracking-wider">
                            <tr>
                                ...
                                <th className="p-5">KETERANGAN BIMBINGAN & KONSELING</th>
                            ...
                            {filteredGuidances.length === 0 ? (
                                <tr>
                                    <td colSpan="4" className="p-16 text-center text-slate-400 font-bold uppercase tracking-wider">
                                        ...
                                        Belum ada rekam bimbingan konseling yang terdaftar
                                    </td>
                                </tr>
                            ) : (
