...
import { HeartHandshake, ChevronUp, ChevronDown, Check, RefreshCcw, Filter, Inbox, Loader2, Calendar, User, Eye, Edit2, Trash2, X } from 'lucide-react';
...
export default function DosenBimbinganKonselingWrapper() {
    ...
    const initData = useCallback(async () => {
        try {
            ...
            const [resStudents, resHistory] = await Promise.all([
                ...
                axios.get('https://api.sigmaeducation.id/api/lecturer/history-guidance-counselings', { headers })
            ]);
    ...
    const handleSubmit = async () => {
        if (!formData.user_id || !formData.tanggal || !formData.keterangan) {
            return Swal.fire("Data Tidak Lengkap", "Silakan pilih nama residen dan isi keterangan bimbingan.", "warning");
        }
        ...
        try {
            ...
            if (isEditMode) {
                ...
                await axios.put(`https://api.sigmaeducation.id/api/lecturer/update-guidance-counseling/${editId}`, formData, { headers });
                Swal.fire("Berhasil Diperbarui", "Data bimbingan residen telah berhasil diubah.", "success");
                handleCancelEdit();
            } else {
                // Eksekusi API Simpan Baru (POST)
                await axios.post('https://api.sigmaeducation.id/api/lecturer/store-guidance-counseling', formData, { headers });
                Swal.fire("Berhasil Disimpan", "Aktivitas bimbingan residen telah dicatat & diparaf otomatis.", "success");
                setFormData({
                    user_id: '',
                    tanggal: new Date().toISOString().split('T')[0],
                    keterangan: ''
            ...
            const resHistory = await axios.get('https://api.sigmaeducation.id/api/lecturer/history-guidance-counselings', { headers });
            setHistoryGuidances(Array.isArray(resHistory.data) ? resHistory.data : []);
        } catch (err) {
    ...
    const handleDeleteClick = (id, studentName) => {
        Swal.fire({
            title: "Hapus Rekam Bimbingan?",
            text: `Apakah Anda yakin ingin menghapus data rekam bimbingan dari dokter residen ${studentName}? Tindakan ini permanen.`,
            icon: "warning",
            ...
            if (result.isConfirmed) {
                try {
                    const headers = { Authorization: `Bearer ${localStorage.getItem('token')}` };
                    await axios.delete(`https://api.sigmaeducation.id/api/lecturer/delete-guidance-counseling/${id}`, { headers });
                    
                    Swal.fire("Dihapus", "Data rekam bimbingan berhasil dihapus dari logbook.", "success");
    ...
    if (isPageLoading) return (
        <div className="w-full h-screen flex flex-col items-center justify-center bg-slate-50 gap-2">
            ...
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Sinkronisasi Lembar Konseling...</p>
        </div>
    );
            ...
            <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h1 className="text-xl font-black text-slate-800 tracking-tight flex items-center gap-2 uppercase">
                        <HeartHandshake className="text-blue-600" size={24} /> Pencatatan Bimbingan & Konseling
                    </h1>
                    <p className="text-xs font-bold text-slate-400 mt-1">
                        Input mandiri bimbingan penalaran, minat, bakat, dan kesejahteraan untuk Residen bimbingan Anda.
                    </p>
                </div>
            ...
            <div className={`bg-white rounded-[32px] shadow-sm border transition-all duration-300 ${isEditMode ? 'border-amber-200 bg-amber-50/10' : 'border-slate-200/60'} mb-8 overflow-hidden`}>
                <div className="p-5 border-b border-slate-100 flex justify-between items-center">
                    <h2 className="flex items-center gap-2 font-black text-slate-800 uppercase text-xs tracking-wider">
                        ...
                        {isEditMode ? `Mode Edit: Koreksi Logbook` : `Formulir Input Logbook Residen`}
                    </h2>
                        ...
                        <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="overflow-hidden">
                            <div className="p-8 grid grid-cols-1 md:grid-cols-3 gap-6">
                                ...
                                <div className="md:col-span-3 pt-2">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 px-1 block">Keterangan Pengembangan (Penalaran, Minat, Bakat, Seni, Kesejahteraan)</label>
                                    <textarea rows="3" placeholder="Tuliskan catatan bimbingan atau konseling hasil pertemuan dengan residen di sini..." className="custom-input resize-none" value={formData.keterangan} onChange={e => setFormData({...formData, keterangan: e.target.value})}></textarea>
                                </div>
                            </div>
            ...
            <div className="bg-white rounded-[32px] shadow-sm border border-slate-200/60 overflow-hidden">
                <div className="p-5 border-b border-slate-100 flex items-center justify-between gap-4 flex-wrap">
                    <h2 className="font-black text-slate-800 uppercase tracking-tight text-xs flex items-center gap-2">
                        <Filter size={16} className="text-slate-400" /> Lembar Logbook Riwayat Bimbingan Aktif
                    </h2>
                    <input 
                        type="text" 
                        placeholder="Cari nama residen atau isi keterangan..." 
                    ...
                    <table className="w-full text-left border-none">
                        <thead className="bg-slate-50/60 text-slate-400 text-[10px] font-black uppercase border-b border-slate-100 tracking-wider">
                            <tr>
                                ...
                                <th className="p-5 w-64">RESIDEN</th>
                            ...
                            {filteredHistory.length === 0 ? (
                                <tr>
                                    <td colSpan="6" className="p-16 text-center text-slate-400 font-bold uppercase tracking-wider">
                                        <Inbox size={28} className="mx-auto mb-2 opacity-50" /> Belum ada rekam bimbingan konseling yang dicatat
                                    </td>
                                </tr>
                            ) : (
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" onClick={() => setShowModal(false)}>
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-[32px] shadow-2xl w-full max-w-lg overflow-hidden border border-slate-100" onClick={e => e.stopPropagation()}>
                            <div className="bg-[#003178] p-6 text-white border-none">
                                <span className="text-[9px] font-black uppercase tracking-widest opacity-60 block mb-1">Logbook Bimbingan & Konseling</span>
